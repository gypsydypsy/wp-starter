...
function seopress_404_set_messages($messages) {
	...
	if ('seopress_404' === $typenow) {
		...
		$messages[$post_type] = [
			...
			1 => /* translators: %1$s singular name of the post type, %2$s button to test the redirect */ sprintf(esc_html__('%1$s updated. %2$s'), esc_html($singular), $seopress_404_test),
			2 => esc_html__('Custom field updated.'),
			3 => esc_html__('Custom field deleted.'),
			4 => /* translators: %1$s singular name of the post type, %2$s button to test the redirect */ sprintf(esc_html__('%1$s updated. %2$s'), esc_html($singular), $seopress_404_test),
			5 => isset($_GET['revision']) ? /* translators: %1$s singular name of the post type, %2$s title of the revision */ sprintf(esc_html__('%1$s restored to revision from %2$s'), esc_html($singular), wp_post_revision_title((int) $_GET['revision'], false)) : false,
			6 => /* translators: %1$s singular name of the post type, %2$s button to test the redirect */ sprintf(esc_html__('%1$s published. %2$s'), esc_html($singular), $seopress_404_test),
			7 => esc_html__('Redirection saved.'),
			8 => /* translators: %s singular name of the post type */ sprintf(esc_html__('%s submitted.'), esc_html($singular)),
			9 => /* translators: %1$s singular name of the post type, %2$s scheduled date */ sprintf(esc_html__('%1$s scheduled for: <strong>%2$s</strong>. '), esc_html($singular), date_i18n(esc_html__('M j, Y @ G:i'), strtotime($post->post_date)), esc_url(get_permalink($post_ID))),
			10 => /* translators: %s singular name of the post type */ sprintf(esc_html__('%s draft updated.'), esc_html($singular)),
		];
...
function seopress_404_set_messages_list($bulk_messages, $bulk_counts) {
	$bulk_messages['seopress_404'] = [
		'updated' => /* translators: %d number of redirections */ _n('%d redirection updated.', '%d redirections updated.', $bulk_counts['updated']),
		'locked' => /* translators: %d number of redirections */ _n('%d redirection not updated, somebody is editing it.', '%d redirections not updated, somebody is editing them.', $bulk_counts['locked']),
		'deleted' => /* translators: %d number of redirections */ _n('%d redirection permanently deleted.', '%d redirections permanently deleted.', $bulk_counts['deleted']),
		'trashed' => /* translators: %d number of redirections */ _n('%d redirection moved to the Trash.', '%d redirections moved to the Trash.', $bulk_counts['trashed']),
		'untrashed' => /* translators: %d number of redirections */ _n('%d redirection restored from the Trash.', '%d redirections restored from the Trash.', $bulk_counts['untrashed']),
	];
...
function seopress_404_count_columns($columns) {
	...
	$columns['seopress_404_redirect_referer'] = esc_html__('Referer', 'wp-seopress-pro');
