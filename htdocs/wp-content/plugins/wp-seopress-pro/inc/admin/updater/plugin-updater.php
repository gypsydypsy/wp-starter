...
class SEOPRESS_Updater
{
    ...
    public function show_update_notification($file, $plugin)
    {
        ...
        if ( ! current_user_can('update_plugins')) {
            ...
            echo ' ';
            /* translators: %1$s: a tag, %2$s changelog link, %3$s version number, %4$s closing a tag, %5$s upgrade link */
            printf(__('%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'wp-seopress-pro'), '<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url($changelog_link) . '">', esc_html($update_cache->response[ $this->name ]->new_version), '</a>', '<a target="_blank" class="update-link" href="' . esc_url(wp_nonce_url($update_link, 'upgrade-plugin_' . $file)) . '">', '</a>');
        } else {
            printf(' %1$s%2$s%3$s', '<a target="_blank" class="update-link" href="' . esc_url(wp_nonce_url($update_link, 'upgrade-plugin_' . $file)) . '">', esc_html__('Update now.', 'wp-seopress-pro'), '</a>');
        }
    ...
    public function show_changelog()
    {
        ...
        if ( ! current_user_can('update_plugins')) {
            wp_die(esc_html__('You do not have permission to install plugin updates', 'wp-seopress-pro'), esc_html__('Error', 'wp-seopress-pro'), [ 'response' => 403 ]);
        }
