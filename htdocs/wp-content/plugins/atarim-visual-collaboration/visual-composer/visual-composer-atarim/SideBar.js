...
import ActivateAtarim from './ActivateAtarim';
import AtarimLicence from './ActivateAtarimLicense';
...
export default function Sidebar(props) {
  ...
  const handleSearchTitle = (e) => {
    if (Object.keys(taskData).length !== 0) {
      ...
      let result = taskData.data.filter(each=>removeTags(each.task.task_title).toLowerCase().includes(titleSearch.toLowerCase()) )
  ...
  return (
    <>
    {
      isLoadingTask ?
        ...
        isNoSiteId || isNotlicense ?
        <>
          {isNoSiteId && 
          <ActivateAtarim
            pluginData = {pluginData}
            textDesc= "To start collaborating, click the button below to begin a quick setup wizard and unlock visual collaboration within minutes inside Visual Composer, getting your clients, team and you, all on the same page (literally)."
            buttonText= "Start The Wizard"
            buttonLink= "/wp-admin/admin.php?page=wpfeedback_page_settings"
          /> }
          {isNotlicense && 
          <ActivateAtarim
            pluginData = {pluginData}
            textDesc= "It looks like your site is deactivated, jump back in less than a minute to start visually collaborating by clicking the button below."
            buttonText= "Reactivate This Website"
            buttonLink= "/wp-admin/admin.php?page=wpfeedback_page_permissions"
          />}
        </>:
        <>
                  ...
                  >
                      ...
                      <TaskContentWrapper>
                          <TaskAuthor>
                              ...
                              <TaskDate>{task?.task?.task_time}</TaskDate>
                          </TaskAuthor>
                          <TaskText
