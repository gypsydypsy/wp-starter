...
function relativeToAbsolute($url, $relative)
{
    ...
    $pm = wp_parse_url('http://1/' . $relative);
...
if (empty($_SERVER['HTTP_HOST'])) {
    ...
    $tmp = createTmpFile( sanitize_url( $_GET['url'] ), false );
    ...
    if ($tmp === false) {
        ...
        $response = H2CP_PREFER_CURL && function_exists('curl_init') ?
            curlDownloadSource( sanitize_url( $_GET['url'] ), $tmp['source'] ) :
            downloadSource( sanitize_url( $_GET['url'] ), $tmp['source'], 0 );
